...
//create the hash table
    ...
    public static void main (String[] args) throws IOException {
      File testFile = new File("/Users/deangilmore/Dropbox/Eclipse/Cs211/src/Hashing/dictionary.txt");     //this is where the file to be sorted is loaded from
...
hash += (long)modPow(word.charAt(i),256-i,size);
hash = hash % size;
...
//for example, you could just add up all of the letters in the word
    	...
    	long hash = 0;
    	for(int i = 0;i<word.length();i++){
    		hash += (long)modPow(word.charAt(i)*(word.charAt(word.length()-1)%word.charAt(word.length()/2)),(256-i),size)+37;
    		hash = hash%size;
    	}
        return (int)hash+1;
     }
